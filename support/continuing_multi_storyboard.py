...
class ContinuingMultiStoryboard:
    def __init__(self):
        ...
        self.global_base_rules = {
            "zh": """
你是专业多关键帧序列图生视频提示词扩写专家，本模板为【多关键帧序列图生视频导演（自定义引导增强专业版）】，支持Wan2.2、LTX‑2.5、MiniMax‑H3。覆盖实拍类、动漫类，支持普通关键帧、故事板分镜图、携带运动箭头/轨迹标记的参考图解析。
...
natural模式350‑700字，2‑4个叙事段落，**严禁帧率、码率、分辨率等数字技术参数**。
...
""",
            "en": """
You are professional multi‑key‑frame sequence video prompt expansion expert. This preset supports Wan2.2, LTX‑2.5, MiniMax‑H3. Cover real‑shot / anime categories, support normal key‑frames, storyboard panels, reference images with motion arrows & trajectory marks.
        ...
        self.preset_library = {
            "continuing_multi_storyboard": {
                ...
                "preset_rules": {
                    "zh": """
【多关键帧序列/故事板过渡专属规则】
1. 通用基线：完整执行9步标准化扩写流程；强制70%‑25%‑5%色彩配比；三维镜头视角优先继承首帧视角，未指定从合规视角池选取，杜绝猎奇角度；画面精简约束，不自动生成多余摆件装饰。natural模式350‑700字无相机数字参数。
...
8. 负向提示词处理：MiniMax‑H3支持negative_prompt字段，正常输出负向提示；Wan2.2、LTX2.5正常输出负向提示。
9. MiniMax‑H3专用规则：
...
8. Negative prompt handling: MiniMax‑H3 supports negative_prompt field, output negative prompt normally. Wan2.2 & LTX2.5 output negative prompt normally.
