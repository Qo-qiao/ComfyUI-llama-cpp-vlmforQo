...
class ContinuingI2V:
    def __init__(self):
        # 视频下游模型配置库：Wan2.2 / LTX2.5 / MiniMax‑H3
        ...
        self.preset_library = {
            "continuing_i2v": {
                ...
                "preset_rules": {
                    "zh": """
【图生视频首帧延续专属规则】
1. 通用基线：执行8步I2V扩写流程；强制70%‑25%‑5%色彩配比；严格锁定首帧全部锚点（主体位置、服装、场景、光影、外形）；三维镜头视角优先继承首帧视角；镜头运动优先固定或极慢运动；画面精简约束，不自动生成多余摆件装饰。natural模式300‑600字无相机数字参数。
...
1. General baseline: follow 8‑step I2V expansion workflow. Mandatory color ratio 70%‑25%‑5%. Lock all reference‑frame anchor(subject position, costume, scene, light‑shadow, appearance). Inherit camera view from reference‑frame whenever possible. Prefer static or extremely‑slow camera movement. Frame‑simplify rule: no auto‑generate extra ornaments. Natural mode 300‑600 words without numeric camera params.
...
**【MiniMax‑H3专用输出格式】若视频模型为MiniMax‑H3，忽略上方自然段落格式，强制按四字段输出：\nintegrated_multimodal_description:\n（画面描述、[Shot N]镜头编号与At时间戳、镜头运动、角色动作序列、台词<d>[语言]…</d>、画内音乐）\n\noverall_soundscape:\n（全片环境音+人物动作音效+非语言人声，1‑4句一个连续段落；完全静音才写N/A）\n\nnon_diegetic_music:\n（画外配乐，1‑3句，描述乐器/速度/节奏/动态变化；无BGM写N/A）\n\nnegative_prompt:\n（需规避的缺陷列表，如 多余路人, 肢体扭曲, 穿模, 面部崩坏, 模糊, 水印）\n\n关键约束：I2VA指令行必须放在第一行（"For the target video, at 0.00 seconds into the target video, <Picture 1> (from [Shot 1]) is fully referenced."），指令行后空一行；首镜头[Shot 1]无需时间戳（默认起始00:00.000）且开头声明风格与初始构图，后续每个[Shot N]搭配At时间戳，最后一个时间戳严格对应视频总时长；人物动作音效与环境音统一放overall_soundscape，对话/歌声/画内音乐放integrated字段，严禁交叉重复；禁止复合镜头运动优先固定机位，运镜写明类型+幅度+速度；人物锚点至少3个具体特征禁止"或\
"字二选一；禁止百分比坐标/色彩占比数字/画面比例参数；所有"不出现XX"类否定约束必须同时写入negative_prompt。""",
