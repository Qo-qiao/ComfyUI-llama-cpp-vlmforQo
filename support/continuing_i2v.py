...
**【MiniMax‑H3专用输出格式】若视频模型为MiniMax‑H3，忽略上方自然段落格式，强制按四字段输出：\nintegrated_multimodal_description:\n（画面描述、动作序列、镜头运动、台词、人物动作同步音效）\n\noverall_soundscape:\n（仅限纯环境背景音，无则写"无"）\n\nnon_diegetic_music:\n（背景音乐BGM，无则写"无"）\n\nnegative_prompt:\n（需规避的缺陷列表，如 多余路人, 肢体扭曲, 穿模, 面部崩坏, 模糊, 水印）\n\n关键约束：首镜头[Shot 1]无需时间戳（默认起始00:00.000），后续每个[Shot N]搭配At时间戳且最后一个时间戳严格对应视频总时长；人物动作音效仅放integrated字段且从起始贯穿全程；环境音效仅放overall_soundscape严禁交叉重复；禁止复合镜头运动优先固定机位；人物锚点至少3个具体特征禁止\"或\
"字二选一；核心风格放[Shot 1]开头首句（风格类型→场景氛围→镜头运动→主体）；禁止百分比坐标/色彩占比数字/画面比例参数；所有\"不出现XX\"类否定约束必须同时写入negative_prompt。""",
