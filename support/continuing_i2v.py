    ...
    def __init__(self):
        ...
        self.video_model_formula_library = {
            "Wan2.2": {
                "keyword_dense": True,
                "mix_lang": False,
                "formula_zh": "Wan2.2模型约束：原生单段最大时长5秒，优先2‑4秒；仅支持单镜头，一个片段控制1‑2个核心微动作，禁止大幅度位移；不生成音频，禁止写台词音效；支持负向提示词。内容组织顺序：首帧锚点延续约束 → 场景基调光影 → 主体微动态 → 镜头运动 → 色彩质感 → 风格标签。natural模式可以使用逗号分隔关键词或连贯短段落；禁止描述音频台词，禁止单片段多镜头切换。",
                "formula_en": "Wan2.2 constraint: native max clip 5s, recommend 2‑4s. Single shot only, keep 1‑2 core micro‑actions per clip, forbid large‑scale displacement. No audio output, do not write dialogue or sound‑effect. Support negative prompt. Content order: reference‑frame anchor constraint → scene‑lighting → character micro‑motion → camera movement → color‑texture → style tags. Allow comma‑separated tags or short paragraphs. Forbid audio description and multi‑cut inside one clip."
            },
            "LTX2.3": {
                "keyword_dense": False,
                "mix_lang": False,
                "formula_zh": "LTX‑2.3模型约束：原生单段最长20秒，稳定推荐3‑12秒；提示词字数必须匹配时长；原生支持音画同步，可以写台词、环境音、背景音乐；支持负向提示词；单段为连续长镜头，禁止写镜头剪辑切镜指令。内容组织顺序：首帧锚点延续约束 → 镜头设定与场景光影 → 主体时序微动态 → 镜头运动节奏 → 色彩质感 → 音频描述 → 风格标签。natural模式输出流畅叙事段落，不要大量逗号碎片化标签堆砌。",
                "formula_en": "LTX‑2.3 constraint: native max 20s, stable recommend 3‑12s. Prompt length must match clip duration. Native audio‑visual sync, allow dialogue, ambient sound, background music. Support negative prompt. Single clip is one continuous long‑take, forbid cut‑edit instruction. Content order: reference‑frame anchor constraint → camera‑scene‑lighting → character chronological micro‑motion → camera rhythm → color‑texture → audio description → style tags. Use coherent narrative paragraphs, avoid mass comma‑split tags."
            },
            "MiniMax‑H3": {
                "keyword_dense": False,
                "mix_lang": False,
                "formula_zh": "MiniMax‑H3模型约束：时长仅支持4‑15秒整数秒，不支持小数秒；**不支持负向提示词**，缺陷规避约束全部写入正向提示词；原生音画同步，支持台词、环境音、BGM；支持首帧参考图；单段连续镜头，禁止写切镜剪辑。内容组织顺序：前置缺陷规避约束 → 首帧锚点延续约束 →镜头设定场景光影 →主体时序微动态 →镜头运动 →色彩质感 →音频描述 →风格标签。natural模式使用连贯叙事段落；不要输出negative字段，所有避坑写进正向文本。",
                "formula_en": "MiniMax‑H3 constraint: only integer seconds 4‑15s, float second invalid. **No negative prompt field**, all defect‑avoid rules must be written into positive prompt text. Native audio‑visual sync, support dialogue, ambient sound, BGM. Support reference image. Single continuous shot, forbid cut instruction. Content order: precondition(defect avoidance) → reference‑frame anchor constraint → camera‑scene‑lighting → character chronological micro‑motion → camera movement → color‑texture → audio → style tags. Use coherent paragraphs, do not output negative prompt field."
            }
        }
