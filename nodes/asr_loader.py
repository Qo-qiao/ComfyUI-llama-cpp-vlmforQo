...
# ISO 639-1 代码 -> Qwen3-ASR 语言全称（兼容旧工作流中保存的代码别名）
...
def _normalize_language(language):
    """将 ISO 代码别名归一化为 Qwen3-ASR 语言全称，auto/全称保持不变"""
...
class omni_llm_asr_loader:
    ...
    def INPUT_TYPES(s):
        ...
        qwen3_languages = [
            ...
            # ISO 639-1 代码别名（兼容旧工作流中保存的代码，如 'zh'，内部会归一化为全称）
    ...
    def _transcribe_qwen(self, audio_input, language):
        ...
        try:
            if hasattr(self.model, 'transcribe'):
                ...
                lang_for_model = _normalize_language(language)
                
                results = self.model.transcribe(
                    audio=audio_input_qwen,
                    language=None if lang_for_model == "auto" else lang_for_model,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "omni_llm_asr_loader": "Omni LLM ASR Loader",
}